...
export async function uploadFile(bucket: string, name: string, bytes: Buffer): Promise<string> {
  if (insforgeConfigured()) {
    try {
      const out = await api<{ url: string }>(`/api/storage/buckets/${bucket}/objects/${name}`, {
        method: "PUT",
  ...
  // Offline: files already live in /public — return the local path convention.
  return `/${bucket}/${name}`;
}
