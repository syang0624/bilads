...
import type { AdConcept, GenerateRequest, GenerateResponse } from "@/types";
...
import { generateAdImage, PLACEHOLDER_IMAGE } from "@/lib/images";
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const imageUrls = await Promise.all(
      drafts.map((d, i) => generateAdImage(safeImagePrompt(d.imagePrompt, board), cacheKey, i))
    );
    ...
    if (cached) {
      ...
      // 3) … or canned copy templates + placeholder image path.
      const drafts = fallbackConcepts({ productName: body.brief.productName, board });
      response = { concepts: drafts.map((d) => toConcept(d, PLACEHOLDER_IMAGE)) };
      path = "canned";
    }
  }
